...
process_event :: proc(e: sdl.Event, state: ^SDL_State) {
    ...
    #partial switch e.type {
        ...
        case .MOUSEBUTTONDOWN: {
            if e.button.button == sdl.BUTTON_LEFT   do state.mouse_down[0] = true;
            if e.button.button == sdl.BUTTON_RIGHT  do state.mouse_down[1] = true;
            if e.button.button == sdl.BUTTON_MIDDLE do state.mouse_down[2] = true;
        }

        case .KEYDOWN, .KEYUP: {
            sc := e.key.keysym.scancode;
            io.keys_down[sc] = e.type == .KEYDOWN;
            io.key_shift = sdl.GetModState() & {sdl.Keymod.LSHIFT, sdl.Keymod.RSHIFT} != nil;
            io.key_ctrl  = sdl.GetModState() & {sdl.Keymod.LCTRL, sdl.Keymod.RCTRL}   != nil;
            io.key_alt   = sdl.GetModState() & {sdl.Keymod.LALT, sdl.Keymod.RALT}     != nil;

            when ODIN_OS == "windows" {
                io.key_super = false;
            } else {
                io.key_super = sdl.GetModState() & {sdl.Keymod.LGUI, sdl.Keymod.RGUI} != nil;
            }
        }
    }
}
...
update_display_size :: proc(window: ^sdl.Window) {
    ...
    if .MINIMIZED in transmute(sdl.WindowFlags) sdl.GetWindowFlags(window) {
        w = 0;
        h = 0;
    }
